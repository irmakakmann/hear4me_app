...
class Tensor {
  ...
  Uint8List get data {
    // Native pointer to the tensor's bytes
    final dataPtr = cast<Uint8>(tfliteBinding.TfLiteTensorData(_tensor));
    final tensorByteSize = tfliteBinding.TfLiteTensorByteSize(_tensor);

    checkState(isNotNull(dataPtr), message: 'Tensor data is null.');

    // View the native memory as a typed list, then copy it to a regular Dart Uint8List
    final externalTypedData = dataPtr.asTypedList(tensorByteSize);
    return Uint8List.fromList(externalTypedData);
  }
