...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget _buildHomeContent(BuildContext context, VoidCallback onOpenLive) {
    return SafeArea(
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const WatchStatusCard(isConnected: true, batteryPercent: 75),
            const SizedBox(height: 16),
            const SoundLevelCard(currentDb: 62, thresholdDb: 85),
            const SizedBox(height: 18),
            const _SectionTitle('Quick Actions'),
            const SizedBox(height: 8),
            _QuickActionsRow(onOpenLive: onOpenLive),
          ],
        ),
      ),
    );
  ...
  static const List<String> _appBarTitles = <String>[
    'Hear4Me', 'Sounds', 'History', 'Profile', 'Settings',
  ];
    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        items: const <BottomNavigationBarItem>[
          ...
          BottomNavigationBarItem(icon: Icon(Icons.graphic_eq), label: 'Sounds'),
          BottomNavigationBarItem(icon: Icon(Icons.history), label: 'History'),
          BottomNavigationBarItem(icon: Icon(Icons.person), label: 'Profile'),
          BottomNavigationBarItem(icon: Icon(Icons.bar_chart), label: 'Settings'),
        ],
...
class WatchStatusCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bgGreen = const Color(0xFFE8F5E9);
    final borderGreen = const Color(0xFF66BB6A);
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(isConnected ? 'Watch Connected' : 'Disconnected',
                    style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: green)),
                const SizedBox(height: 2),
                const Text('Hearing Aid Watch', style: TextStyle(fontSize: 14, color: Colors.black54)),
              ],
            ),
          ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Text('Current Sound Level', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w800)),
          const SizedBox(height: 12),
          Text('$currentDb', style: TextStyle(fontSize: 48, fontWeight: FontWeight.w700, color: valColor)),
          const Text('dB', style: TextStyle(fontSize: 20, color: Colors.black54)),
          const SizedBox(height: 8),
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              ...
              Text('Alert threshold: $thresholdDb dB',
                  style: TextStyle(fontSize: 16, color: over ? Colors.red : Colors.black54)),
            ],
          ),
        ],
      ),
...
class _QuickActionsRow extends StatelessWidget {
  ...
  const _QuickActionsRow({required this.onOpenLive});
...
class _QuickActionCard extends StatelessWidget {
  ...
  const _QuickActionCard({required this.icon, required this.iconColor, required this.label, this.onTap});
